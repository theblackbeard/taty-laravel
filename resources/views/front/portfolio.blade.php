...
@section('title') @if(!isset($results)) Trabalhos com {{str_replace("-"," ",$name)}} @else Todos os Trabalhos @endif @endsection
@section('content')
<section id="portfolio">
    @if(count($articles) > 0)
        <ul>
            <li><a href="/main" >Todos os Projetos</a></li>

            @if(isset($results))
                @foreach($results as $result)
                    <li class="menu_category"><a href="/portfolio/{{ str_replace(" ", "-", $result)}}">{{$result}}</a></li>
                @endforeach
            @endif
        </ul>
        <div class="row">
